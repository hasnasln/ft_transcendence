...
export function renderRegister(container) {
    ...
    const showLoginBtn = registerFooter.querySelector("#show-login");
    ...
    registerForm.onsubmit = async (e) => {
        ...
        if (!username || !email || !password || !repeatPassword) {
            if (!username)
                errorDiv.textContent = exmp.getLang('register-errors.required.username');
            else if (!email)
                errorDiv.textContent = exmp.getLang('register-errors.required.email');
            else if (!password)
                errorDiv.textContent = exmp.getLang('register-errors.required.password');
            else if (!repeatPassword)
                errorDiv.textContent = exmp.getLang('register-errors.required.confirmPassword');
            errorDiv.style.visibility = "visible";
            return;
        }
        if (username.length < 3 || password.length < 6) {
            if (username.length < 3)
                errorDiv.textContent = exmp.getLang('register-errors.minlength.username');
            else if (password.length < 6)
                errorDiv.textContent = exmp.getLang('register-errors.minlength.password');
            errorDiv.style.visibility = "visible";
            return;
        }
        if (username.length > 20 || password.length > 20) {
            if (username.length > 20)
                errorDiv.textContent = exmp.getLang('register-errors.maxlength.username');
            else if (password.length > 20)
                errorDiv.textContent = exmp.getLang('register-errors.maxlength.password');
            errorDiv.style.visibility = "visible";
            return;
        }
