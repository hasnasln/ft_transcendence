...
export class Settings {
    isOpen = false;
    selectedColor = localStorage.getItem('ballColor') || '#3b82f6';
    ...
    onLoad() {
        const settingsContainer = document.getElementById('content-container');
        if (!settingsContainer) {
            console.error('Content container not found in show');
            return;
        }
        settingsContainer.innerHTML += this.evaluate();
        const overlay = document.getElementById('settings_main');
    ...
    initializeEventListeners() {
        ...
        if (!grid)
            console.log('onload da aranan grid bulunamadı');
        else {
        ...
        const container = document.getElementById('dropdownHTML');
        const currentBtn = container.querySelector('#current-lang-btn');
        const dropdownMenu = container.querySelector('#dropdown-menu-lang');
        currentBtn.addEventListener('click', (e) => {
            this.toggleDropdown(dropdownMenu, currentBtn);
        });
        dropdownMenu.querySelectorAll('button[data-lang]').forEach((btn) => {
            btn.addEventListener('click', () => {
                this.tmpLanguage = btn.getAttribute('data-lang'); // geçici olarak seçilen dili sakla
        ...
        if (!footer)
            console.log('onload da aranan footer bulunamadı');
        else {
    ...
    selectColor(color, grid, colors) {
        ...
        grid.querySelectorAll('.group\\/card').forEach((card, index) => {
            ...
            if (isSelected)
                card.innerHTML += `${createEnhancedCheckmarkV2()}`;
        });
    }
    toggleDropdown(dropdownMenu, currentBtn) {
    ...
    closeDropdown(dropdownMenu, currentBtn) {
        const arrow = currentBtn.querySelector('svg');
    ...
    selectLanguage(lang, currentBtn, dropdownMenu, flags, names, langs) {
        ...
        dropdownMenu.querySelectorAll('button[data-lang]').forEach((btn) => {
            btn.addEventListener('click', () => {
                this.tmpLanguage = btn.getAttribute('data-lang');
                this.selectLanguage(this.tmpLanguage, currentBtn, dropdownMenu, flags, names, langs);
            });
        });
        this.closeDropdown(dropdownMenu, currentBtn);
    }
    updateLanguageDisplay(lang, flags, names) {
        return `
...
function createEnhancedCheckmarkV2() {
    return `
