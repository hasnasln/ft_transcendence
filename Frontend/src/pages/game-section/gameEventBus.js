...
export class GameEventBus {
    ...
    offAllByLabel(label) {
        for (const eventType of gameEventTypes) {
            this.offByFilter(eventType, h => h.labels.includes(label));
        }
    }
    offByFilter(eventType, filter) {
...
export function listenGameBusEvents() {
    ...
    GameEventBus.getInstance().on('RIVAL_FOUND', (event) => {
        const matchPlayers = event.payload.matchPlayers;
        const rival = matchPlayers.left.socketId === WebSocketClient.getInstance().getSocket().id ? matchPlayers.right.username : matchPlayers.left.username;
    ...
    GameEventBus.getInstance().on('RECONNECTED', () => {
        if (!gameInstance.gameStatus.currentGameStarted || !gameInstance.uiManager.isSceneReady())
            return;
    ...
    GameEventBus.getInstance().on('BALL_PADDLE_HIT', event => {
        if (!gameInstance.uiManager.ball)
            return;
