...
export class ProfileSettings {
    ...
    password;
    isInitialized = false;
    static globalEventListenerAdded = false;
    constructor() {
    ...
    render(container) {
        ...
        window.currentProfileInstance = this;
    ...
    setupEventListener(container) {
        if (ProfileSettings.globalEventListenerAdded)
            return;
        ...
        const bodyClickHandler = (event) => {
            ...
            if (!target.closest('#profile_main'))
                return;
            const action = this.findActionFromEvent(event);
            if (action) {
                event.preventDefault();
    ...
    async hendle_password(action) {
        const eski_sifre = document.querySelector('#' + action + '_eski');
        const new_pass = document.querySelector('#' + action);
        if (!eski_sifre) {
    ...
    close() {
        ...
        if (profileMain) {
            const sidebar = profileMain.querySelector('div:last-child');
...
function createProfileSettings(container, settings) {
    ...
    settings.forEach(({ name, type, placeholder, action }) => {
        ...
        saveButton.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            const profileInstance = window.currentProfileInstance || new ProfileSettings();
            if (action === 'password') {
...
function createProfileInfoSection() {
...
function createProfileSettingsSection() {
    const section = document.createElement('div');
    section.className = `
		group relative bg-gradient-to-br from-white/95 via-slate-50/90 to-white/95
		backdrop-blur-3xl rounded-[2rem] p-12 border-2 border-white/50
...
function getUserBasedAvatar(username) {
    let hash = 0;
...
function showAvatarSelector(currentImageElement) {
...
function selectAvatar(avatarName, imageElement, modal) {
...
function showAvatarChangeSuccess() {
...
class ToastNotification {
    ...
    static show(message, type = 'success', duration = TIMING.TOAST_DURATION) {
