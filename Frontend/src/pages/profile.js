...
export class Profile {
    ...
    onLoad() {
        ...
        const overlay = document.getElementById('profile_main');
        const sidebar = document.getElementById('profile_sidebar');
        ...
        const profileImageWraper = document.getElementById('profile-image-container');
        profileImageWraper.addEventListener('click', () => {
            this.showAvatarSelector();
        });
        const second_part = document.getElementById('second_part');
    ...
    async hendle_password(action) {
        const old_pass = document.querySelector('#' + action + '_eski');
    ...
    onloadAvatarModal(modal) {
        ...
        const avatarGridClose = document.getElementById('avatar-modal-cancel');
        avatarGridClose.addEventListener('click', () => {
            this.closeModal();
        });
        const avatarGridv2 = document.getElementById('avatar-girid-v2');
        avatarGridv2.addEventListener('click', (e) => {
            const target = e.target.closest('button[data-avatar]');
            if (!target)
                return;
    ...
    selectAvatar(avatarName) {
        const imageElement = document.getElementById('current-avatar');
    ...
    createProfileInfoSection() {
        ...
        if (!currentAvatar)
            currentAvatar = this.getUserBasedAvatar(this.username);
        return `
		<div
