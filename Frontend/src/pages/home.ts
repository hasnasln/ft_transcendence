...
export class HomePage implements Page {
	...
	evaluate(): string {
        ...
        const profileCards = profiles.map(profile => profileCard(
            profile.name,
            profile.linkedin,
            profile.github,
            profile.avatar
	...
	return `
		<li class="group relative w-14 h-14 overflow-hidden rounded-2xl transition-all duration-700 ease-out hover:w-80 hover:shadow-2xl hover:shadow-purple-500/30 cursor-pointer bg-gradient-to-br from-white/90 to-gray-50/90 backdrop-blur-2xl border border-white/30 hover:border-purple-300/60 hover:from-white hover:to-purple-50/50 transform hover:scale-105 hover:-translate-y-1 ring-1 ring-white/20 hover:ring-purple-300/40" data-action="${action}">
			...
			<button data-action="${action}" class="relative w-full h-full flex items-center px-4 py-3 ${textColor} font-semibold transition-all duration-700 ease-out focus:outline-none focus:ring-2 focus:ring-purple-500/50 rounded-2xl">
				...
				<div data-action="${action}" class="flex-shrink-0 w-6 h-6 ml-auto transform translate-x-full opacity-0 group-hover:translate-x-0 group-hover:opacity-100 transition-all duration-700 delay-200 text-purple-500 drop-shadow-sm">
...
function getPlayIconHTML(): string {
    return `
...
function getSettingsIconHTML(): string {
    return `
...
function getProfileIconHTML(): string {
    return `
...
function getTournamentIconHTML(): string {
    return `
...
function getExitIconHTML(): string {
    return `
