...
export class TournamentValidation {
    validateTournamentStatus(data, status) {
    ...
    async askTournamentConfirmation(title, message, acceptText = "", cancelText = "") {
    ...
    async confirmTournamentStart(playerCount) {
    ...
    async confirmTournamentExit(isAdmin) {
        let title;
        let message;
        let confirmText;
        const cancelText = exmp.getLang('tournament-confirmation.cancel');
