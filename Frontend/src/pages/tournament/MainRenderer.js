...
export function ShowTournament(container, tdata) {
    container.innerHTML = createTournamentPageHTML(tdata);
    const playersList = container.querySelector('#list-player');
    if (playersList) {
        listPlayers(playersList, tdata);
    }
}
function createTournamentPageHTML(tdata) {
    return `
...
function createHeaderSection(tdata) {
    return `
...
function createInfoPanel(tdata) {
    return `
...
function createDetailsCard(tdata) {
    return `
...
function getAdminUsername(tdata) {
...
function getTournamentTimeDisplay(tdata) {
...
function createActionButton(action, icon, color, tooltip) {
    return `
        <button
...
function createInfoCard(title_key, value_key, icon, gradient, customSubtitle) {
...
function createAdminPanel(tdata) {
...
function createStartButton(canStart, playerCount, minPlayers) {
...
function createStartInfo(canStart, playerCount) {
    if (canStart) {
        return `
...
function findByeParticipant(round) {
    if (!round.winners || round.winners.length === 0) {
        return undefined;
    }
    for (const winner of round.winners) {
        const isInMatches = round.matches.some((match) => match.participant1.uuid === winner.uuid ||
...
function isPlayButtonCreated(ttada) {
    switch (ttada.status) {
        ...
        case TournamentStatus.ONGOING:
            {
                if (ttada.tournament_start) {
                    let round_tmp;
                    ttada.tournament_start.rounds.forEach((round) => {
                    ...
                    if (round_tmp.is_completed) {
                        if (ttada.participants.some((p) => p.uuid === uid)) {
                            return true;
                        }
                        return false;
                    }
...
function createPlayersPanel(tdata) {
    return `
...
export function getOptimalTournamentSize(playerCount) {
    const sizes = [2, 4, 8, 16];
    return sizes.find(size => size >= playerCount) || 16;
}
export function calculateByes(playerCount) {
    return getOptimalTournamentSize(playerCount) - playerCount;
}
export function getTournamentFormat(playerCount) {
    const formats = {
        2: "Tek maç",
        4: "Yarı final + Final",
...
export function listPlayers(container, tdata) {
    container.innerHTML = createPlayersListHTML(tdata);
}
function createPlayersListHTML(tdata) {
    const playersHTML = tdata.lobby_members.map((player, index) => createPlayerCard(player, index, tdata.admin_id)).join('');
    return `
        ${playersHTML}
        ${createCapacityIndicator(tdata.lobby_members.length)}
    `;
}
function createPlayerCard(player, index, adminId) {
...
function createPlayerAvatar(initial) {
    return `
...
function createPlayerInfo(username, isAdmin) {
...
function createPlayerStats(playerNumber) {
    return `
...
function createCapacityIndicator(currentCapacity) {
