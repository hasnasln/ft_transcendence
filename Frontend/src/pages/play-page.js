...
function renderScoreBoard(container) {
    ...
    // Konumlandırma
    'absolute', 'top-[5%]', 'left-[67%]', '-translate-x-1/2', 
    // Görünmez başlangıç
    'hidden', 
    // Flex ayarları
    'flex', 'justify-center', 'items-center', 
    ...
    // Konumlandırma
    'absolute', 'top-[5%]', 'left-[30%]', '-translate-x-1/2', 
    // Görünmez başlangıç
    'hidden', 
    // Flex ayarları
    'flex', 'justify-center', 'items-center', 
...
function createDivAddId(id) {
    const x = document.createElement('div');
    x.id = id;
    return x;
}
function createSpanAddId(id) {
    const x = document.createElement('span');
    x.id = id;
    return x;
}
function renderSetBoard(container) {
    const setboard = createDivAddId('setboard');
    setboard.classList.add('absolute', 'bottom-[3%]', 'left-1/2', 'transform', '-translate-x-1/2', 'mt-[0.3vw]', 'hidden', 'flex', 'flex-col', 'justify-center', 'items-center', 'px-[2vw]', // padding: 0vw 2vw
    'py-0', 
    // Özel gradient 
    'bg-gradient-to-br', 'from-[#1e1e1e]', 'to-[#2c2c2c]', 'border', 'border-2', 'border-[#555]', 'rounded-[12px]', 
    // Yakın bir box-shadow 
    'shadow-[0_0_10px_rgba(255,255,255,0.2),0_0_20px_rgba(255,255,255,0.1)]', 'font-sans', 'text-[1.4vw]', 'text-[#eee]', 'z-10');
    // Başlık kısmı
    ...
    setboard00.classList.add('text-[1.3vw]');
    ...
    setboard10.classList.add('text-green-500');
    setboard11.textContent = "0 : 0";
    setboard11.classList.add('px-[1vw]', 'py-[0.2vw]');
    setboard12.textContent = "P2";
    setboard12.classList.add('text-green-500');
...
function createButtonWithNameId(id, Name) {
...
function renderMenu(container) {
    ...
    menudiv.classList.add('absolute', 'top-1/2', 'left-1/2', '-translate-x-1/2', '-translate-y-1/2', 'w-[60%]', 'h-[70%]', 'bg-black', 'rounded-3xl', 'flex', 'flex-col', 'justify-center', 'items-center', 'gap-8', // butonlar arası boşluk
    'z-10', 'hidden');
    const buttonData = [
...
function renderDifficulty(container) {
    ...
    difficulty.classList.add('absolute', 'top-1/2', 'left-1/2', '-translate-x-1/2', '-translate-y-1/2', 'w-[50%]', 'h-[60%]', 'bg-gray-300', 'rounded-3xl', 'flex', 'flex-col', 'justify-center', 'items-center', 'gap-4', 'z-10', 'hidden');
...
export class PlayPage {
    ...
    render(container) {
        ...
        paly_div.classList.add('bg-gray-300', 'w-[100%]', 
        //'h-[100%]',
        'relative', 'flex', 'justify-center', 'items-center', 'flex-col');
        const canvas = document.createElement('canvas');
        canvas.id = 'game-canvas';
        canvas.classList.add("w-[90%]");
        ...
        set_toast.classList.add('absolute', 'bottom-[20%]', 'left-1/2', '-translate-x-1/2', 'bg-[#222]', 'text-white', 'text-[1.5vw]', 'px-[2vw]', 'py-[1vw]', 'rounded-[8px]', 'z-10', 'hidden');
        end_message.classList.add('absolute', 'top-[30%]', 'left-1/2', '-translate-x-1/2', '-translate-y-1/2', 'bg-[#000000cc]', 'text-white', 'text-[2vw]', 'px-[4vw]', 'py-[2vw]', 'rounded-[12px]', 'z-10', 'hidden');
        info.classList.add('absolute', 'top-[20%]', 'left-1/2', '-translate-x-1/2', 'bg-[#222]', 'text-white', 'text-[1.8vw]', 'px-[2vw]', 'py-[1vw]', 'rounded-[8px]', 'z-10', 'hidden');
        ...
        [start_Button, devambutton, new_match, turnToHomePage].forEach(btn => {
            btn.classList.add('absolute', 'left-1/2', '-translate-x-1/2', '-translate-y-1/2', 'px-[2.8vw]', 'py-[1.2vw]', 'text-[1.5vw]', 'text-white', 'border-none', 'rounded-[12px]', 'cursor-pointer', 'z-30', 'bg-blue-500', 'hover:bg-blue-700', 'hover:scale-105', 'transition-all', 'transform', 'hidden');
        });
