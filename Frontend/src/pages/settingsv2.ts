...
export class Settingsv2 {
	...
	private createSettingsUI(): void {
		...
		const content = this.createContentV2();
		const footer = this.createFooterV2();
	...
	private createContentV2(): HTMLElement {
		...
		wrapper.firstElementChild?.firstElementChild?.appendChild(this.createColorSectionV2());
		wrapper.firstElementChild?.firstElementChild?.appendChild(this.createLanguageSectionV2());
		return wrapper.firstElementChild as HTMLElement;
	}

	private createColorSectionV2(): HTMLElement {
		const colorSectionHTML = `
			<div class="group relative bg-gradient-to-br from-white/95 via-slate-50/90 to-white/95
				...
				<div id="color-grid-placeholder"></div>
			</div>
		`;

		const wrapper = document.createElement('div');
		wrapper.innerHTML = colorSectionHTML;
		const section = wrapper.firstElementChild as HTMLElement;

		const colorGrid = this.createColorGridV2();
		const placeholder = section.querySelector('#color-grid-placeholder');
		if (placeholder) {
			placeholder.replaceWith(colorGrid);
		}
	...
	private createColorGridV2(): HTMLElement {
		const colors = [
		...
		let gridHTML = `<div class="grid grid-cols-4 gap-6 relative z-10 max-w-lg mx-auto">`;
		...
		colors.forEach((color) => {
			...
			gridHTML += `
				...
				>
					...
					${isSelected ? createEnhancedCheckmarkV2().outerHTML : ''}
	...
	private selectColor(color: string, grid: HTMLElement, colors: string[]): void {
		...
		grid.querySelectorAll('.group\\/card').forEach((card, index) => {
			...
			if (isSelected) {
				const checkmark = createEnhancedCheckmarkV2();
				card.appendChild(checkmark);
			}
		});
	}

	private createLanguageSectionV2(): HTMLElement {
		const languageSectionHTML = `
			<div class="group relative bg-gradient-to-br from-white/95 via-slate-50/90 to-white/95
				...
				<!-- Language dropdown burada eklenecek -->
				<div id="language-dropdown-placeholder"></div>
			</div>
		`;

		const wrapper = document.createElement('div');
		wrapper.innerHTML = languageSectionHTML;
		const section = wrapper.firstElementChild as HTMLElement;

		const dropdown = this.createLanguageDropdownV2();
		const placeholder = section.querySelector('#language-dropdown-placeholder');
		if (placeholder) {
	...
	private createLanguageDropdownV2(): HTMLElement {
        ...
        <div class="relative z-[100] dropdown-container">
            ...
            >
                <div class="flex items-center gap-3">
                    <div class="text-xl group-hover:scale-110 transition-transform duration-300">${langFlags[this.selectedLanguage] || "🌐"}</div>
                    <div class="text-left">
                        <div class="font-bold text-gray-800 text-sm">${langNames[this.selectedLanguage] || this.selectedLanguage}</div>
	...
	private createFooterV2(): HTMLElement {
    const footerHTML = `
        <div class="p-10 bg-gradient-to-br from-white/90 via-slate-50/80 to-white/90 
    ...
    const footer = wrapper.firstElementChild as HTMLElement;
...
function createEnhancedCheckmarkV2(): HTMLElement {
    const html = `
...
function createHeaderV2(): HTMLElement {
	const headerHTML = `
