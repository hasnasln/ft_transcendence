export function creaters(container, top, left, range, name) {
    const img = document.createElement('img');
    img.src = `/creaters/${name}.png`;
    const createrDiv = document.createElement('div');
    createrDiv.id = 'creater_div';
    createrDiv.classList.add('w-[400px]', 'h-[300px]', 'rounded-full', 'bg-gray-500', 'absolute', `${left}`, `${top}`, 'overflow-hidden');
